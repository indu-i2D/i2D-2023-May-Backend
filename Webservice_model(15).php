...
class Webservice_model extends CI_Model{
    ...
    function __construct()
    {
		...
		date_default_timezone_set("America/Indiana/Indianapolis"); // Set Default Time Zone
		//date_default_timezone_set("Asia/Kolkata"); // Set Default Time Zone
        // Added line below 2023-03-13
        error_reporting(0);
    }
    ...
    public function welcomeMail($details)
    {
            $to = $details['email'];
            $subject = "Welcome to i2Donate";
                                        ...
                                        <tr>
                                            <td>
                                                <div>
                                                    ...
                                                    <p><strong> Welcome to i2-Donate. We welcome you to the world of socially responsible giving.</strong></p>
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
...
public function updateProfile($details)
	{
                $this->db->select(" * ");
                $where = array(DB_prefix.'EMAIL' => $details['email'], DB_prefix.'ID != ' => $details['user_id']);
                $this->db->where($where);
		$query = $this->db->get(DB_prefix.'users');		                
                if($query->num_rows()>0) {		
			$data = array('status'=>0, 'message'=>'Email address already exists');
                        return $data;
		}
		else
                {
	...
	{
		//$clientToken = Braintree_ClientToken::generate();
               // the next two lines were added on Jan 22, 2023 to send Amount + processing Fee to braintree for processing fee.
                $braintree_amount = $details['amount']+$details['processing_fee'];
		$result = $this->braintree_trans($braintree_amount, $details['transaction_id']);
               //End of lines added
		...
		$insert_data = array(
			...
			DB_prefix.'CHARITY_NAME' => $details['charity_name'],
			DB_prefix.'TRANSACTION_ID' => $details['transaction_id'],
                        DB_prefix.'DONATION_AMOUNT' => $details['amount'],
	      //	DB_prefix.'AMOUNT' => $details['amount'],
              // The Following 2 lines added on Jan 22, 2023
		        DB_prefix.'MERCHANT_CHARGES' => $details['merchant_charges'],
			DB_prefix.'PROCESSING_FEE' => $details['processing_fee'],
              //	End of Lines added 
			DB_prefix.'STATUS' => $details['status'],
			DB_prefix.'CREATED_AT' => date('Y-m-d H:i:s')
		);
	...
	public function changePassword($details)
	{
		...
		$data = $query->row_array();
		if(count($data)>0){
                    if(strcmp($details['old_password'], $details['new_password'])!=0)
                    {
    		$update_data = array(
    		...
    		$data = array('status'=>1, 'message'=>'Password has been changed', 'token_status'=>1, 'token_message'=>'Valid token');
                       ...
                       $data = array('status'=>0, 'message'=>'Your old password and new password are same.', 'token_status'=>1, 'token_message'=>'Valid token');
                    }   
		}
		else {
    ...
    public function charityList($details)
    {
        ...
        if($details['country_code'] == 'INT')
        {

            $where.= DB_prefix."COUNTRY_CODE='INT' and ";
        }
        if($details['country_code'] == 'US' || empty($details['country_code']))
        {
        ...
        if($details['country_code'] == 'US' || empty($details['country_code']))
        {
            if(!empty($city))
            {
                $where.= " AND (".DB_prefix."STATE_NAME LIKE '".$city."%' OR ".DB_prefix."CITY LIKE '%".$city."%')";
            }
        ...
        if($details['country_code'] == 'INT')
        {
            if(!empty($city))
            {
                $where.= " AND (".DB_prefix."COUNTRY_NAME LIKE '".$city."%')";
                ///$where.= " AND ".DB_prefix."COUNTRY_NAME LIKE '%".$city."%'";
    ...
    //MAX(CASE WHEN c.meta_key = 'country' THEN (select ".DB_prefix."NAME FROM iactscon_2018_countries_lists where sortname=meta_value) END) AS country,
        ...
        if($data)
        {
            ...
            $data = array('status'=>0, 'message'=>'No Records Found', 'data'=> array(), "aa" => "SELECT ".DB_prefix."ID as id, ".DB_prefix."NAME as name, ".DB_prefix."STREET as street, ".DB_prefix."CITY as city, ".DB_prefix."STATE as state, ".DB_prefix."ZIP as zip_code, ".DB_prefix."COUNTRY_CODE as country,(select count(".DB_prefix."ID) FROM ".DB_prefix."charity_likes where ".DB_prefix."CHARITY_ID=id) as like_count, (select count(".DB_prefix."ID) FROM ".DB_prefix."charity_likes where ".DB_prefix."CHARITY_ID=id AND ".DB_prefix."USER_ID = ".$details['user_id'].") as liked, (select count(".DB_prefix."ID) FROM ".DB_prefix."following where ".DB_prefix."CHARITY_ID=id AND ".DB_prefix."USER_ID = ".$details['user_id'].") as followed,(select count(".DB_prefix."ID) FROM ".DB_prefix."following where ".DB_prefix."CHARITY_ID=id ) as followed_count".DB_prefix."LOGO as logo, ".DB_prefix."TAX_DEDUCTIBLE as taxdeductible, ".DB_prefix."INCOME_AMT as amt FROM ".DB_prefix."nonprofits where ".$where.$asc_order.$asc_order1.$limit);
        }
        return $data;
    }
	...
	/* MySpace User Likes Following Payment */
	public function likesAndFollowings($details)
	{
		...
		for($i=0; $i<count($data); $i++)
		{
			...
			$payment_history_query = $this->db->query("SELECT ".DB_prefix."DONATION_AMOUNT as amount,DATE_FORMAT(".DB_prefix."CREATED_AT,'%d/%m/%Y %h:%i %p') as donate_date  FROM ".DB_prefix."payments_data a  where a.".DB_prefix."USER_ID = '". $details['user_id']."' AND a.".DB_prefix."CHARITY_ID = ".$data[$i]['id']);
